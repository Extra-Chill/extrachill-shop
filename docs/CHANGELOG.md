...
### Changed
- declare the PHP version this plugin actually needs
- adopt the shared Homeboy release train
- isolate commerce state behind owner contracts

### Fixed
- exclude editor config from the release package scope
- correct the stale Requires PHP header
- correct composer.json PHP floor to match union-type syntax in use
- authorize paid priority boost fulfillment
- preserve blog context for priority boosts

## [0.9.1] - 2026-07-05

### Changed
- consume commerce credentials via the network-layer filter contract

## [0.9.0] - 2026-07-04

### Added
- open artist self-serve wp-admin gates

### Changed
- store Stripe + Shippo credentials via DM encrypted auth provider
