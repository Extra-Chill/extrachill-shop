<?php
/**
 * Lifetime Membership WooCommerce Integration
 *
 * Adds username field and checkout validation for the lifetime membership product.
 * Grants the membership on order completion by delegating to ec_create_lifetime_membership().
 *
...
add_filter( 'woocommerce_add_to_cart_validation', 'extrachill_shop_validate_lifetime_membership_add_to_cart', 10, 3 );
...
add_action( 'woocommerce_payment_complete', 'extrachill_shop_auto_complete_lifetime_membership_order', 20 );
add_action( 'woocommerce_order_status_completed', 'extrachill_shop_handle_lifetime_membership_purchase', 10 );

function extrachill_shop_is_lifetime_membership_product_id( $product_id ) {
	$lifetime_membership_product_id = function_exists( 'extrachill_shop_get_lifetime_membership_product_id' )
		? absint( extrachill_shop_get_lifetime_membership_product_id() )
		: 0;

	return $lifetime_membership_product_id && absint( $product_id ) === $lifetime_membership_product_id;
}
...
function extrachill_shop_add_community_username_field() {
	...
	if ( ! extrachill_shop_is_lifetime_membership_product_id( $product->get_id() ) ) {
		return;
	}
	...
	<div class="community-username-field">
		...
		<p class="description">Enter the community username that should receive the lifetime membership.</p>
	</div>
	<?php
}
...
function extrachill_shop_validate_lifetime_membership_add_to_cart( $passed, $product_id, $quantity ) {
	...
	if ( empty( $username ) ) {
		wc_add_notice( 'Please enter a valid Community Username for the Lifetime Membership.', 'error' );
		return false;
	}
...
function extrachill_shop_display_username_cart( $item_data, $cart_item ) {
	if ( empty( $cart_item['product_id'] ) || ! extrachill_shop_is_lifetime_membership_product_id( $cart_item['product_id'] ) ) {
		return $item_data;
	}
...
function extrachill_shop_auto_complete_lifetime_membership_order( $order_id ) {
	if ( ! $order_id ) {
		return;
	}
	...
	if ( ! extrachill_shop_is_lifetime_membership_product_id( $item->get_product_id() ) ) {
		return;
	}

	$order->update_status( 'completed', 'Auto-completed: Lifetime Membership only.' );
}

function extrachill_shop_handle_lifetime_membership_purchase( $order_id ) {
	if ( ! $order_id ) {
		return;
	}
	...
	if ( $order->get_meta( '_ec_lifetime_membership_processed', true ) ) {
		return;
	}
	...
	foreach ( $order->get_items() as $item ) {
		...
		$order->update_meta_data( '_ec_lifetime_membership_processed', 1 );
		$order->add_order_note( "Lifetime Extra Chill Membership granted for username: {$username}." );
		$order->save();
		break;
	}
}
